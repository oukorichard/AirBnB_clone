...
class HBNBCommand(cmd.Cmd):
    """class HBHB to read command"""
    prompt = 'hbnb '
    __all_117 = 0

    def emptyline(self):
        """pass if no information is given"""
        pass

    def precmd(self, line):
        """edits given command to allow second type of input"""
    ...
    def do_create(self, arg):
        "create instance if the model exists"
        if not arg:
    ...
    def do_show(self, arg):
        "print dict of an instance in the base of it's ID"
        cmd_argv = arg.split()
        if not cmd_argv:
            print("**class name missing**")
            return None
